...
static int sam0_eic_init(const struct device *dev)
{
	...
	sys_set_bit(mclk + MCLK_APBAMASK_OFFSET, MCLK_APBAMASK_EIC_BIT);

#if !defined(CONFIG_SOC_SERIES_SAMD20) && !defined(CONFIG_SOC_SERIES_SAMD21) &&                    \
	!defined(CONFIG_SOC_SERIES_SAMR21)

	/* Enable the GCLK */
	sys_write32(GCLK_EIC_CLK_EN, gclk + PCHCTRL_OFFSET + (4 * EIC_GCLK_IDX));
#else
	/* Enable the GCLK */
	sys_write16(GCLK_EIC_CLK_EN, gclk + GCLK_EIC_CLK_OFFSET);
#endif

...
static void wait_synchronization(const uintptr_t usart)
{
	...
	while ((sys_read32(usart + USART_SYNCBUSY_OFFSET) & USART_SYNCBUSY_MASK) != 0) {
	}
#else
	/* SYNCBUSY is a bit */
	while (sys_test_bit(usart + USART_STATUS_OFFSET, USART_STATUS_SYNCBUSY_BIT)) {
	}
#endif
}
				  ...
				  uint32_t clk_freq_hz)
{
	...
	sys_write16(baud, usart + USART_BAUD_OFFSET);
				  ...
				  uint32_t id, int error_code)
{
	...
	sys_write8(BIT(USART_INTENSET_TXC), cfg->regs + USART_INTENSET_OFFSET);
}
				  ...
				  uint32_t id, int error_code)
{
	...
	dma_reload(cfg->dma_dev, cfg->rx_dma_channel,
		   (uint32_t)(cfg->regs + USART_DATA_OFFSET),
	...
	if (dev_data->rx_timeout_time != SYS_FOREVER_US) {
		...
		sys_write8(USART_INTENSET_RXC, cfg->regs + USART_INTENSET_OFFSET);
		irq_unlock(key);
		return;
	}
			       ...
			       const struct uart_config *new_cfg)
{
	...
	sys_clear_bit(usart + USART_CTRLA_OFFSET, USART_CTRLA_ENABLE_BIT);
	wait_synchronization(usart);
	...
	uint32_t CTRLA_temp = sys_read32(usart + USART_CTRLA_OFFSET);
	uint32_t CTRLB_temp = sys_read32(usart + USART_CTRLB_OFFSET);

	switch (new_cfg->parity) {
	case UART_CFG_PARITY_NONE:
		WRITE_BIT(CTRLA_temp, USART_CTRLA_FORM_BIT, 0);
		break;
	case UART_CFG_PARITY_ODD:
		WRITE_BIT(CTRLA_temp, USART_CTRLA_FORM_BIT, 1);
		WRITE_BIT(CTRLB_temp, USART_CTRLB_PMODE_BIT, 1);
		break;
	case UART_CFG_PARITY_EVEN:
		WRITE_BIT(CTRLA_temp, USART_CTRLA_FORM_BIT, 1);
		WRITE_BIT(CTRLB_temp, USART_CTRLB_PMODE_BIT, 0);
		break;
	default:
		return -ENOTSUP;
	}
	...
	case UART_CFG_STOP_BITS_1:
		WRITE_BIT(CTRLB_temp, USART_CTRLB_SBMODE_BIT, 0);
		break;
	case UART_CFG_STOP_BITS_2:
		WRITE_BIT(CTRLB_temp, USART_CTRLB_SBMODE_BIT, 1);
		break;
	default:
		return -ENOTSUP;
	}
	...
	CTRLB_temp &= ~USART_CTRLB_CHSIZE_MASK;
	switch (new_cfg->data_bits) {
	case UART_CFG_DATA_BITS_5:
		CTRLB_temp |= USART_CTRLB_CHSIZE(0x5);
		break;
	case UART_CFG_DATA_BITS_6:
		CTRLB_temp |= USART_CTRLB_CHSIZE(0x6);
		break;
	case UART_CFG_DATA_BITS_7:
		CTRLB_temp |= USART_CTRLB_CHSIZE(0x7);
		break;
	case UART_CFG_DATA_BITS_8:
		CTRLB_temp |= USART_CTRLB_CHSIZE(0x0);
		break;
	case UART_CFG_DATA_BITS_9:
		CTRLB_temp |= USART_CTRLB_CHSIZE(0x1);
		break;
	default:
		return -ENOTSUP;
	}
	...
	WRITE_BIT(CTRLB_temp, USART_CTRLB_COLDEN_BIT, cfg->pads);
#endif

	sys_write32(CTRLA_temp, usart + USART_CTRLA_OFFSET);
	wait_synchronization(usart);

	sys_write32(CTRLB_temp, usart + USART_CTRLB_OFFSET);
	wait_synchronization(usart);
	...
	sys_set_bit(usart + USART_CTRLA_OFFSET, USART_CTRLA_ENABLE_BIT);
	wait_synchronization(usart);
...
static int uart_sam0_init(const struct device *dev)
{
	...
	*cfg->mclk |= cfg->mclk_mask;
	...
	sys_write32(GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN(cfg->gclk_gen),
		    gclk + GCLK_PCHCTRL_OFS + (4 * cfg->gclk_id));
#else
	sys_write16(GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN(cfg->gclk_gen) |
			    GCLK_CLKCTRL_ID(cfg->gclk_id),
		    gclk + GCLK_CLKCTRL_OFS);
#endif

	/* Disable all USART interrupts */
	sys_write8(USART_INTENCLR_MASK, usart + USART_INTENCLR_OFFSET);
	wait_synchronization(usart);
	...
	uint32_t reg =
	    cfg->pads
	    /* Internal clock */
	    | USART_CTRLA_MODE(0x1)
#ifndef CONFIG_SOC_SERIES_SAMD20
	    /* 16x oversampling with arithmetic baud rate generation */
	    | USART_CTRLA_SAMPR(0)
#endif
	    | USART_CTRLA_FORM(0) |
	    BIT(USART_CTRLA_CPOL_BIT) | BIT(USART_CTRLA_DORD_BIT);
	sys_write32(reg, usart + USART_CTRLA_OFFSET);
	wait_synchronization(usart);
	...
	sys_write32(USART_CTRLB_CHSIZE(0) | USART_CTRLB_RXEN | USART_CTRLB_TXEN,
			usart + USART_CTRLB_OFFSET);
	wait_synchronization(usart);
	...
	if (cfg->tx_dma_channel != 0xFFU) {
		...
		dma_blk.dest_address = (uint32_t)(usart + USART_DATA_OFFSET);
	...
	if (cfg->rx_dma_channel != 0xFFU) {
		...
		dma_blk.block_size = 1;
		dma_blk.source_address = (uint32_t)(usart + USART_DATA_OFFSET);
...
static int uart_sam0_poll_in(const struct device *dev, unsigned char *c)
{
	...
	if (!(sys_read8(config->regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_RXC)) {
		return -EBUSY;
	}

	*c = (unsigned char)sys_read8(config->regs + USART_DATA_OFFSET);
	return 0;
}
...
static void uart_sam0_poll_out(const struct device *dev, unsigned char c)
{
	...
	while (!(sys_read8(config->regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_DRE)) {
	}

	/* send a character */
	sys_write16(c, config->regs + USART_DATA_OFFSET);
}
...
static int uart_sam0_err_check(const struct device *dev)
{
	...
	uint32_t err = 0U;
	uint16_t status = sys_read16(config->regs + USART_STATUS_OFFSET);

	if (IS_BIT_SET(status, USART_STATUS_BUFOVF_BIT)) {
		err |= UART_ERROR_OVERRUN;
	}

	if (IS_BIT_SET(status, USART_STATUS_FERR_BIT)) {
		err |= UART_ERROR_PARITY;
	}

	if (IS_BIT_SET(status, USART_STATUS_PERR_BIT)) {
		err |= UART_ERROR_FRAMING;
	}

#if defined(SERCOM_REV500)
	if (IS_BIT_SET(status, USART_STATUS_ISF_BIT)) {
		err |= UART_BREAK;
	}

	if (IS_BIT_SET(status, USART_STATUS_COLL_BIT)) {
		err |= UART_ERROR_COLLISION;
	}

	status |= BIT(USART_STATUS_BUFOVF_BIT) | BIT(USART_STATUS_FERR_BIT) |
		  BIT(USART_STATUS_PERR_BIT) | BIT(USART_STATUS_COLL_BIT) |
		  BIT(USART_STATUS_ISF_BIT);
#else
	status |= BIT(USART_STATUS_BUFOVF_BIT) | BIT(USART_STATUS_FERR_BIT) |
		  BIT(USART_STATUS_PERR_BIT);
#endif
	sys_write16(status, config->regs + USART_STATUS_OFFSET);
...
static void uart_sam0_isr(const struct device *dev)
{
	...
	if (dev_data->tx_len && (sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_TXC)) {
		sys_write8(USART_INTENCLR_TXC, regs + USART_INTENCLR_OFFSET);
	...
	if (dev_data->rx_len && (sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_RXC) &&
	    dev_data->rx_waiting_for_irq) {
		dev_data->rx_waiting_for_irq = false;
		sys_write8(USART_INTENCLR_RXC, regs + USART_INTENCLR_OFFSET);
			       ...
			       const uint8_t *tx_data, int len)
{
	...
	if ((sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_DRE) && len >= 1) {
		sys_write16(tx_data[0], regs + USART_DATA_OFFSET);
		return 1;
	} else {
		return 0;
	}
...
static void uart_sam0_irq_tx_enable(const struct device *dev)
{
	...
	sys_write8(USART_INTENSET_DRE | USART_INTENSET_TXC, config->regs + USART_INTENSET_OFFSET);
}
...
static void uart_sam0_irq_tx_disable(const struct device *dev)
{
	...
	sys_write8(USART_INTENCLR_DRE | USART_INTENCLR_TXC, config->regs + USART_INTENCLR_OFFSET);
}
...
static int uart_sam0_irq_tx_ready(const struct device *dev)
{
	...
	return ((sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_DRE) &&
	       (sys_read8(regs + USART_INTENSET_OFFSET) & USART_INTENSET_DRE));
}
...
static int uart_sam0_irq_tx_complete(const struct device *dev)
{
	...
	return (dev_data->txc_cache != 0) &&
		((sys_read8(regs + USART_INTENSET_OFFSET) & USART_INTENSET_DRE) != 0);
}
...
static void uart_sam0_irq_rx_enable(const struct device *dev)
{
	...
	sys_write8(USART_INTENSET_RXC, config->regs + USART_INTENSET_OFFSET);
}
...
static void uart_sam0_irq_rx_disable(const struct device *dev)
{
	...
	sys_write8(USART_INTENCLR_RXC, config->regs + USART_INTENCLR_OFFSET);
}
...
static int uart_sam0_irq_rx_ready(const struct device *dev)
{
	...
	return (sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_RXC) != 0;
}
			       ...
			       const int size)
{
	...
	if (sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_RXC) {
		uint8_t ch = sys_read8(regs + USART_DATA_OFFSET);
...
static int uart_sam0_irq_is_pending(const struct device *dev)
{
	...
	return (sys_read8(regs + USART_INTENSET_OFFSET) & sys_read8(regs + USART_INTFLAG_OFFSET)) !=
	       0;
}
...
{
	const struct uart_sam0_dev_cfg *config = dev->config;
	uint8_t flag = sys_read8(config->regs + USART_INTENSET_OFFSET);

	flag |= USART_INTENSET_ERROR;
	sys_write8(flag, config->regs + USART_INTENSET_OFFSET);
	wait_synchronization(config->regs);
}
...
{
	const struct uart_sam0_dev_cfg *config = dev->config;
	uint8_t flag = sys_read8(config->regs + USART_INTENCLR_OFFSET);

	flag |= USART_INTENCLR_ERROR;
	sys_write8(flag, config->regs + USART_INTENCLR_OFFSET);
	wait_synchronization(config->regs);
}
#endif
...
static int uart_sam0_irq_update(const struct device *dev)
{
	...
	dev_data->txc_cache = sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_TXC;
	sys_write8(USART_INTFLAG_ERROR | USART_INTFLAG_RXBRK | USART_INTFLAG_CTSIC |
			   USART_INTFLAG_RXS | dev_data->txc_cache,
		   regs + USART_INTFLAG_OFFSET);
#else
	sys_write8(USART_INTFLAG_RXS, regs + USART_INTFLAG_OFFSET);
#endif
	return 1;
}
			...
			int32_t timeout)
{
	...
	retval = dma_reload(cfg->dma_dev, cfg->tx_dma_channel, (uint32_t)buf,
			    (uint32_t)(regs + USART_DATA_OFFSET), len);
	if (retval != 0U) {
		return retval;
	}
			       ...
			       int32_t timeout)
{
	...
	while (sys_read8(regs + USART_INTFLAG_OFFSET) & USART_INTFLAG_RXC) {
		char discard = sys_read8(regs + USART_DATA_OFFSET);

		(void)discard;
	}

	retval = dma_reload(cfg->dma_dev, cfg->rx_dma_channel,
			    (uint32_t)(regs + USART_DATA_OFFSET),
			    (uint32_t)buf, len);
	if (retval != 0) {
		return retval;
	}
	...
	sys_write8(USART_INTENSET_RXC, regs + USART_INTENSET_OFFSET);
...
#define UART_SAM0_SERCOM_PADS(n) \
	(DT_INST_PROP(n, rxpo) << USART_CTRLA_RXPO_BIT) |		\
	(DT_INST_PROP(n, txpo) << USART_CTRLA_TXPO_BIT)
